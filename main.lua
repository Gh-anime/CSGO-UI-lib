...
XEN.__index = XEN
XEN.Sections = XEN.Sections or {}
XEN.Pages = XEN.Pages or {}
XEN.Sections.__index = XEN.Sections
XEN.Pages.__index = XEN.Pages
...
XEN.Theme = TableClone(Themes["Preset"])

-- Folders
for Index, Value in XEN.Folders do
...
do
    ...
    Tween.Create = function(self, Item, Info, Goal, IsRawItem)
        ...
        Info = Info or TweenInfo.new(XEN.Tween.Time, XEN.Tween.Style, XEN.Tween.Direction)
    ...
    Tween.FadeItem = function(self, Item, Property, Visibility, Speed)
        ...
        local NewTween = Tween:Create(Item, TweenInfo.new(Speed or XEN.Tween.Time, XEN.Tween.Style, XEN.Tween.Direction), {
            [Property] = Visibility and OldTransparency or 1
        }, true)

        XEN:Connect(NewTween.Tween.Completed, function()
...
do
    ...
    Instances.AddToTheme = function(self, Properties)
        ...
        XEN:AddToTheme(self, Properties)
    end
    ...
    Instances.ChangeItemTheme = function(self, Properties)
        ...
        XEN:ChangeItemTheme(self, Properties)
    end
    ...
    Instances.Connect = function(self, Event, Callback, Name)
        ...
        return XEN:Connect(self.Instance[Event], Callback, Name)
    end
    ...
    Instances.MakeDraggable = function(self)
        ...
        XEN:Connect(UserInputService.InputChanged, function(Input)
    ...
    Instances.MakeResizeable = function(self, Minimum, Maximum, Window)
        ...
        XEN:Connect(UserInputService.InputEnded, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                if Resizing then
                    EndResizing()
                end
        ...
        XEN:Connect(RunService.RenderStepped, function()
    ...
    Instances.OnHover = function(self, Function)
        ...
        return XEN:Connect(self.Instance.MouseEnter, Function)
    end
    ...
    Instances.OnHoverLeave = function(self, Function)
        ...
        return XEN:Connect(self.Instance.MouseLeave, Function)
    end
end
...
do
    function CustomFont:New(Name, Weight, Style, Data)
        ...
        writefile(string.format("%s/%s.font", XEN.Folders.Assets, Name), HttpService:JSONEncode(Data))
        return getcustomasset(string.format("%s/%s.font", XEN.Folders.Assets, Name))
    end
...
-- Create holders
XEN.Holder = Instances:Create("ScreenGui", {
    Parent = gethui(),
    Name = "\0",
...
XEN.UnusedHolder = Instances:Create("ScreenGui", {
    Parent = gethui(),
    Name = "\0",
    ZIndexBehavior = Enum.ZIndexBehavior.Global,
    Enabled = false,
...
XEN.NotifHolder = Instances:Create("Frame", {
    Parent = XEN.Holder.Instance,
    Name = "\0",
    BackgroundTransparency = 1,
...
Instances:Create("UIListLayout", {
    Parent = XEN.NotifHolder.Instance,
    Name = "\0",
...
function XEN:Round(Number, Float)
    local Multiplier = 1 / (Float or 1)
    return MathFloor(Number * Multiplier) / Multiplier
end

function XEN:Thread(Function)
...
function XEN:SafeCall(Function, ...)
    local Arguments = { ... }
...
function XEN:Connect(Event, Callback, Name)
...
function XEN:NextFlag()
    local FlagNumber = self.UnnamedFlags + 1
    return StringFormat("flag_number_%s_%s", FlagNumber, HttpService:GenerateGUID(false))
end

function XEN:AddToTheme(Item, Properties)
...
function XEN:ToRich(Text, Color)
    return string.format('<font color="rgb(%d, %d, %d)">%s</font>', MathFloor(Color.R * 255), MathFloor(Color.G * 255), MathFloor(Color.B * 255), Text)
end

function XEN:GetConfig()
    local Config = { }

    local Success, Result = XEN:SafeCall(function()
        for Index, Value in XEN.Flags do
...
function XEN:LoadConfig(Config)
    local Decoded = HttpService:JSONDecode(Config)

    local Success, Result = XEN:SafeCall(function()
        for Index, Value in Decoded do
            local SetFunction = XEN.SetFlags[Index]
...
function XEN:DeleteConfig(Config)
    if isfile(XEN.Folders.Configs .. "/" .. Config) then
        delfile(XEN.Folders.Configs .. "/" .. Config)
    end
end

function XEN:RefreshConfigsList(Element)
    local CurrentList = { }
    local List = { }

    local ConfigFolderName = StringGSub(XEN.Folders.Configs, XEN.Folders.Directory .. "/", "")

    for Index, Value in listfiles(XEN.Folders.Configs) do
        local FileName = StringGSub(Value, XEN.Folders.Directory .. "\\" .. ConfigFolderName .. "\\", "")
        List[Index] = FileName
    end
...
function XEN:ChangeItemTheme(Item, Properties)
    Item = Item.Instance or Item
...
function XEN:ChangeTheme(Theme, Color)
...
function XEN:IsMouseOverFrame(Frame)
...
function XEN:Lerp(Start, Finish, Time)
    return Start + (Finish - Start) * Time
end

function XEN:CompareVectors(PointA, PointB)
    return (PointA.X < PointB.X) or (PointA.Y < PointB.Y)
end

function XEN:IsClipped(Object, Column)
    local Parent = Column
    ...
    return XEN:CompareVectors(Top, BoundryTop) or XEN:CompareVectors(BoundryBottom, Bottom)
end

function XEN:GetCalculatedRayPosition(Position, Normal, Origin, Direction)
    local N = Normal
    local D = Direction
...
function XEN:MakeBlurred(Item, Window)
    Item = Item.Instance
    local BlurItem = Item
    ...
    XEN:Connect(RunService.RenderStepped, function()
        if Window.IsOpen then
            if Item.Visible then
                ...
                local Position0 = XEN:GetCalculatedRayPosition(Origin, Normal, Ray0.Origin, Ray0.Direction)
                local Position1 = XEN:GetCalculatedRayPosition(Origin, Normal, Ray1.Origin, Ray1.Direction)
...
function XEN:EscapePattern(String)
...
do
    function XEN:CreateColorpicker(Data)
        local Colorpicker = {
            Flag = Data.Flag,
        ...
        do
            ...
            if not Data.Parent2.Instance:FindFirstChild("nig") then
                ...
                Items["PaletteIcon"]:OnHover(function()
                    Items["PaletteIcon"]:Tween(nil, { ImageColor3 = XEN.Theme.Accent })
                end)
            ...
            Items["ColorpickerWindow"] = Instances:Create("TextButton", {
                Parent = XEN.UnusedHolder.Instance,
                AutoButtonColor = false,
                Text = "",
                Name = "\0",
                Visible = false,
        ...
        function Colorpicker:Update(IsFromAlpha)
            ...
            if Data.Callback then
                XEN:SafeCall(Data.Callback, Colorpicker.Color, Colorpicker.Alpha)
            end
        end
        ...
        function Colorpicker:SlidePalette(Input)
            ...
            Items["PaletteDragger"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2New(SlideX, 0, SlideY, 0) })
            Colorpicker:Update()
        end
        ...
        function Colorpicker:SlideHue(Input)
            ...
            Items["HueDragger"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2New(SlideX, 0, 0.5, 0) })
            Colorpicker:Update()
        end
        ...
        function Colorpicker:SlideAlpha(Input)
            ...
            Items["AlphaDragger"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2New(SlideX, 0, 0.5, 0) })
        ...
        function Colorpicker:SetOpen(Bool)
            ...
            if Colorpicker.IsOpen then
                ...
                Items["ColorpickerWindow"].Instance.Parent = XEN.Holder.Instance
                ...
                if Data.Section.IsSettings ~= true then
                    for Index, Value in XEN.OpenFrames do
                ...
                XEN.OpenFrames[Colorpicker] = Colorpicker
            else
                if not Data.Section.IsSettings then
                    if XEN.OpenFrames[Colorpicker] then
                        XEN.OpenFrames[Colorpicker] = nil
                    end
                end
            ...
            for Index, Value in Descendants do
                ...
                if type(TransparencyProperty) == "table" then
                    for _, Property in TransparencyProperty do
                        NewTween = Tween:FadeItem(Value, Property, Bool, XEN.FadeSpeed)
                    end
                else
                    NewTween = Tween:FadeItem(Value, TransparencyProperty, Bool, XEN.FadeSpeed)
                end
            end
            ...
            NewTween.Tween.Completed:Connect(function()
                ...
                task.wait(0.2)
                Items["ColorpickerWindow"].Instance.Parent = not Colorpicker.IsOpen and XEN.UnusedHolder.Instance or XEN.Holder.Instance
            end)
        end
        ...
        function Colorpicker:Set(Color, Alpha)
            ...
            Items["PaletteDragger"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2New(PaletteValueX, 0, PaletteValueY, 0) })
            Items["HueDragger"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2New(HuePositionX, 0, 0.5, 0) })
            Items["AlphaDragger"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Position = UDim2New(AlphaPositionX, 0, 0.5, 0) })
            Colorpicker:Update()
        end
        ...
        XEN:Connect(UserInputService.InputBegan, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                if not Colorpicker.IsOpen then
                    return
                end

                if XEN:IsMouseOverFrame(Items["ColorpickerWindow"]) or XEN:IsMouseOverFrame(Items["PaletteIcon"]) and not Data.Section.IsSettings then
                    return
                end
        ...
        XEN.SetFlags[Colorpicker.Flag] = function(Value, Alpha)
    ...
    function XEN:KeybindList(Title)
        local KeybindList = { }
        XEN.KeyList = KeybindList
        ...
        do
            ...
            Instances:Create("UIGradient", {
                ...
                return RGBSequence{ RGBSequenceKeypoint(0, XEN.Theme.Accent), RGBSequenceKeypoint(1, XEN.Theme.AccentGradient) }
            end })
    ...
    function XEN:Notification(Data)
        local Items = { }
        do
    ...
    function XEN:Window(Data)
        ...
        do
            ...
            XEN:MakeBlurred(Items["MainFrame"], Window)
            ...
            XEN:MakeBlurred(Items["LeftTabs"], Window)
            ...
            if IsMobile then
                Items["FloatingButton"] = Instances:Create("TextButton", {
                    ...
                    ZIndex = 127,
                    BackgroundColor3 = XEN.Theme.Background
                })
            ...
            function Window:SetTransparency()
                Items["MainFrame"].Instance.BackgroundTransparency = XEN.Flags["BackgroundTransparency"]
                Items["LeftTabs"].Instance.BackgroundTransparency = XEN.Flags["BackgroundTransparency"]
                if IsMobile then
                    Items["FloatingButton"].Instance.BackgroundTransparency = XEN.Flags["BackgroundTransparency"]
                end

                for _, Value in Items do
                    if _:find("___") then
                        Value.Instance.BackgroundTransparency = tonumber(XEN.Flags["BackgroundTransparency"])
                    end
                end
            end
            ...
            Items["SettingsButton"]:OnHover(function()
                Items["SettingsIconAccent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    Size = UDim2New(1, 0, 1, 0),
            ...
            Items["CloseButton"]:OnHover(function()
                Items["CloseIconAccent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
                    Size = UDim2New(1, 0, 1, 0),
                    BackgroundTransparency = 0
                })
            end)
            ...
            do
                ...
                SettingsItems["CloseButton"]:OnHover(function()
                    SettingsItems["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Size = UDim2New(1, 0, 1, 0), BackgroundTransparency = 0 })
                end)

                SettingsItems["CloseButton"]:OnHoverLeave(function()
                    SettingsItems["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Size = UDim2New(0, 0, 0, 0), BackgroundTransparency = 1 })
                end)
                ...
                function Settings:SetOpen(Bool)
                    ...
                    if Settings.IsOpen then
                        ...
                        SettingsItems["Settings"].Instance.Parent = XEN.Holder.Instance

                        RenderStepped = RunService.RenderStepped:Connect(function()
                            SettingsItems["Settings"].Instance.Position = UDim2New(
                                0, Items["SettingsIcon"].Instance.AbsolutePosition.X,
                                0, Items["SettingsIcon"].Instance.AbsolutePosition.Y + Items["SettingsButton"].Instance.AbsoluteSize.Y + 108)
                        ...
                        XEN.OpenFrames[Settings] = Settings
                    else
                        for Index, Value in Settings.Elements do
                            Value:RefreshPosition(false)
                        end

                        if XEN.OpenFrames[Settings] then
                            XEN.OpenFrames[Settings] = nil
                        end
                    ...
                    NewTween.Tween.Completed:Connect(function()
                        ...
                        task.wait(0.2)
                        SettingsItems["Settings"].Instance.Parent = not Settings.IsOpen and XEN.UnusedHolder.Instance or XEN.Holder.Instance
                    end)
                end
                ...
                setmetatable(Settings, XEN.Sections)
            end

            Settings:Label("First gradient color"):Colorpicker({
                Flag = "AccentColor",
                Default = XEN.Theme.Accent,
                Callback = function(Color)
                    XEN.Theme.Accent = Color
                    XEN:ChangeTheme("Accent", Color)
                end
            })

            Settings:Label("Second gradient color"):Colorpicker({
                Flag = "AccentGradientColor",
                Default = XEN.Theme.AccentGradient,
                Callback = function(Color)
                    XEN.Theme.AccentGradient = Color
                    XEN:ChangeTheme("AccentGradient", Color)
                end
            })
            ...
            Settings:Dropdown({
                ...
                Callback = function(Value)
                    local FontData = XEN.Fonts[Value]
        ...
        task.wait()
        Window:SetOpen(true)
        return setmetatable(Window, XEN)
    end

    function XEN:Category(Name)
        local Items = { }
        do
    ...
    function XEN:Page(Data)
        ...
        function Page:Turn(Bool)
            ...
            Items["Page"].Instance.Parent = Bool and Page.Window.Items["Content"].Instance or XEN.UnusedHolder.Instance
        ...
        return setmetatable(Page, XEN.Pages)
    end

    function XEN.Pages.GlobalChat(self, Side)
        local GlobalChat = { }
        XEN.GlobalChatt = GlobalChat
        ...
        do
            ...
            Items["SendButton"]:OnHover(function()
                Items["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Size = UDim2New(1, 0, 1, 0), BackgroundTransparency = 0 })
            end)

            Items["SendButton"]:OnHoverLeave(function()
                Items["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Size = UDim2New(0, 0, 0, 0), BackgroundTransparency = 1 })
            end)
        ...
        function GlobalChat:SetVisibility(Bool)
            ...
            Items["GlobalChat"].Instance.Parent = Bool and Data.MainFrame.Instance or XEN.UnusedHolder
        end
    ...
    function XEN.Pages.Section(self, Data)
        Data = Data or { }
        ...
        function Section:ToggleBackground()
            ...
            if not Section.IsActive then
                ...
                Items["Toggle"]:Tween(nil, { BackgroundColor3 = XEN.Theme.Element })
                Items["Circle"]:Tween(nil, { AnchorPoint = Vector2New(0, 0.5), Position = UDim2New(0, 4, 0.5, 0), BackgroundColor3 = XEN.Theme.Text, BackgroundTransparency = 0.6 })
            else
                Items["Fade"]:Tween(nil, { BackgroundTransparency = 1 })
                task.spawn(function()
                    task.wait(XEN.Tween.Time)
                    Items["Fade"].Instance.Visible = false
                end)

                Items["Gradient"].Instance.Enabled = true
                Items["Toggle"]:ChangeItemTheme({ BackgroundColor3 = "Text" })
                Items["Toggle"]:Tween(nil, { BackgroundColor3 = XEN.Theme.Text })
                Items["Circle"]:Tween(nil, { AnchorPoint = Vector2New(1, 0.5), Position = UDim2New(1, -4, 0.5, 0), BackgroundColor3 = XEN.Theme.Text, BackgroundTransparency = 0 })
            end
        end

        XEN:Connect(Items["Content"].Instance.Changed, function(Property)
        ...
        return setmetatable(Section, XEN.Sections)
    end

    function XEN.Sections.Toggle(self, Data)
        Data = Data or { }
        ...
        local Toggle = {
            ...
            Flag = Data.Flag or Data.flag or XEN:NextFlag(),
        ...
        function Toggle:Set(Value)
            Toggle.Value = Value
            XEN.Flags[Toggle.Flag] = Value

            if Toggle.Value then
                Items["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.1, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { BackgroundTransparency = 0, Size = UDim2New(1, 0, 1, 0) })
                Items["CheckImage"]:Tween(nil, { ImageTransparency = 0, Size = UDim2New(0, 10, 0, 9) })
            else
                Items["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.05, Enum.EasingStyle.Back, Enum.EasingDirection.Out), { BackgroundTransparency = 1, Size = UDim2New(0, 0, 0, 0) })
                Items["CheckImage"]:Tween(nil, { ImageTransparency = 1, Size = UDim2New(0, 0, 0, 0) })
            end

            if Toggle.Callback then
                XEN:SafeCall(Toggle.Callback, Toggle.Value)
            end
        end
        ...
        function Toggle:Settings(Size)
            ...
            do
                ...
                SettingsItem["Button"]:OnHover(function()
                    SettingsItem["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Size = UDim2New(1, 0, 1, 0), BackgroundTransparency = 0 })
                end)

                SettingsItem["Button"]:OnHoverLeave(function()
                    SettingsItem["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time + 0.15, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), { Size = UDim2New(0, 0, 0, 0), BackgroundTransparency = 1 })
                end)
            end
            ...
            function Settings:SetOpen(Bool)
                ...
                if Settings.IsOpen then
                    ...
                    SettingsItem["Settings"].Instance.Parent = XEN.Holder.Instance
                ...
                NewTween.Tween.Completed:Connect(function()
                    ...
                    task.wait(0.2)
                    SettingsItem["Settings"].Instance.Parent = not Settings.IsOpen and XEN.UnusedHolder.Instance or XEN.Holder.Instance
                end)
            end
            ...
            XEN:Connect(UserInputService.InputBegan, function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                    if XEN:IsMouseOverFrame(SettingsItem["Settings"]) then
                        return
                    end
        ...
        function Toggle:Colorpicker(Data)
            ...
            local NewColorpicker, ColorpickerItems = XEN:CreateColorpicker({
        ...
        function Toggle:Keybind(Data)
            ...
            local NewKeybind, KeybindItems = XEN:CreateKeybind({
                Parent = Items["SubElements"],
                Page = Keybind.Page,
        ...
        Items["Toggle"]:Connect("InputBegan", function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                if Items["SettingsIcon"] and XEN:IsMouseOverFrame(Items["SettingsIcon"]) then
                    return
                end
        ...
        XEN.SetFlags[Toggle.Flag] = function(Value)
            Toggle:Set(Value)
        end
    ...
    function XEN.Sections.Button(self, Data)
        Data = Data or { }
        ...
        function Button:Press()
            ...
            Items["Button"]:Tween(nil, { BackgroundColor3 = XEN.Theme.Accent })
            ...
            XEN:SafeCall(Button.Callback)
            Items["Button"]:ChangeItemTheme({ BackgroundColor3 = "Element" })
            Items["Button"]:Tween(nil, { BackgroundColor3 = XEN.Theme.Element })

            Items["Text"]:Tween(nil, { TextColor3 = XEN.Theme.Text, TextTransparency = 0.3 })

            if Button.Icon then
                Items["Icon"]:Tween(nil, { ImageColor3 = XEN.Theme.Text, ImageTransparency = 0.3 })
            end
        end
    ...
    function XEN.Sections.Slider(self, Data)
        Data = Data or { }
        ...
        function Slider:Set(Value)
            Slider.Value = XEN:Round(MathClamp(Value, Slider.Min, Slider.Max), Slider.Decimals)
            XEN.Flags[Slider.Flag] = Slider.Value

            Items["Accent"]:Tween(TweenInfo.new(XEN.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), { Size = UDim2New((Slider.Value - Slider.Min) / (Slider.Max - Slider.Min), 0, 1, 0) })
            ...
            if Slider.Callback then
                XEN:SafeCall(Slider.Callback, Slider.Value)
            end
        end
        ...
        XEN.SetFlags[Slider.Flag] = function(Value)
            Slider:Set(Value)
        end
    ...
    function XEN.Sections.Dropdown(self, Data)
        Data = Data or { }
        ...
        function Dropdown:SetOpen(Bool)
            ...
            if Dropdown.IsOpen then
                ...
                Items["OptionHolder"].Instance.Parent = XEN.Holder.Instance
                ...
                XEN.OpenFrames[Dropdown] = Dropdown
            else
                ...
                if XEN.OpenFrames[Dropdown] then
                    XEN.OpenFrames[Dropdown] = nil
                end
            ...
            NewTween.Tween.Completed:Connect(function()
                ...
                task.wait(0.2)
                Items["OptionHolder"].Instance.Parent = not Dropdown.IsOpen and XEN.UnusedHolder.Instance or XEN.Holder.Instance
            end)
        end
        ...
        function Dropdown:Set(Option)
            if Dropdown.Multi then
                ...
                XEN.Flags[Dropdown.Flag] = Option
            ...
            if Dropdown.Callback then
                XEN:SafeCall(Dropdown.Callback, Dropdown.Value)
            end
        end
        ...
        function Dropdown:Add(Option)
            ...
            function OptionData:Set()
                ...
                if Dropdown.Multi then
                    ...
                    XEN.Flags[Dropdown.Flag] = Dropdown.Value
                    ...
                    if OptionData.Selected then
                        ...
                        XEN.Flags[Dropdown.Flag] = OptionData.Name
        ...
        XEN:Connect(UserInputService.InputBegan, function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                if Dropdown.IsOpen then
                    if XEN:IsMouseOverFrame(Items["OptionHolder"]) then
                        return
                    end
        ...
        Items["RealDropdown"]:Connect("Changed", function(Property)
            if Property == "AbsolutePosition" and Dropdown.IsOpen then
                Dropdown.IsOpen = not XEN:IsClipped(Items["OptionHolder"].Instance, Dropdown.Section.Items["Section"].Instance.Parent)
                Items["OptionHolder"].Instance.Visible = Dropdown.IsOpen
            end
        end)
        ...
        XEN.SetFlags[Dropdown.Flag] = function(Value)
            Dropdown:Set(Value)
        end
    ...
    function XEN.Sections.Label(self, Name)
        local Label = {
            Window = self.Window,
            Page = self.Page,
            Section = self,
    ...
    function XEN.Sections.Keybind(self, Data)
        Data = Data or { }
        ...
        function Keybind:SetMode(Mode)
            if Mode == "Toggle" then
                ...
                Items["Hold"]:Tween(nil, { TextColor3 = XEN.Theme.Text })

                Items["Always"]:ChangeItemTheme({ TextColor3 = function()
                    return XEN.Theme.Text
                end })
                Items["Always"]:Tween(nil, { TextColor3 = XEN.Theme.Text })
            elseif Mode == "Hold" then
                ...
                Items["Toggle"]:Tween(nil, { TextColor3 = XEN.Theme.Text })
            ...
            if Data.Callback then
                XEN:SafeCall(Data.Callback, Keybind.Toggled)
            end
        end
        ...
        XEN:Connect(UserInputService.InputBegan, function(Input)
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                if XEN:IsMouseOverFrame(Items["KeybindWindow"]) or XEN:IsMouseOverFrame(Items["OptionHolder"]) then
                    return
                end
        ...
        XEN.SetFlags[Keybind.Flag] = function(Value)
            Keybind:Set(Value)
        end
    ...
    function XEN.Sections.Textbox(self, Data)
        Data = Data or { }
        ...
        function Textbox:Set(Value)
            ...
            XEN.Flags[Textbox.Flag] = Value

            if Textbox.Callback then
                XEN:SafeCall(Textbox.Callback, Value)
            end
        end
        ...
        if Textbox.Finished then
            ...
            XEN:Connect(Items["Input"].Instance:GetPropertyChangedSignal("Text"), function()
                Textbox:Set(Items["Input"].Instance.Text)
            end)
        end
        ...
        XEN.SetFlags[Textbox.Flag] = function(Value)
            Textbox:Set(Value)
        end
    ...
    function XEN.Sections.Listbox(self, Data)
        Data = Data or { }
        ...
        XEN:Connect(Items["Search"].Instance:GetPropertyChangedSignal("Text"), function()
            XEN:Thread(function()
                for Index, Value in Dropdown.Options do
                    local InputText = Items["Search"].Instance.Text
                    if InputText ~= "" then
                        if StringFind(StringLower(Value.Name), XEN:EscapePattern(StringLower(InputText))) then
...
function XEN:CreateSettingsPage(Window, KeybindList)
    ...
    do
        ...
        ConfigsSection:Button({
            ...
            Callback = function()
                if ConfigName and ConfigName ~= "" then
                    if not isfile(XEN.Folders.Configs .. "/" .. ConfigName .. ".json") then
                        writefile(XEN.Folders.Configs .. "/" .. ConfigName .. ".json", XEN:GetConfig())
                        XEN:RefreshConfigsList(ConfigsDropdown)
                    end
                end
            end
        })
        ...
        ConfigsSection:Button({
            ...
            Callback = function()
                if ConfigSelected then
                    XEN:DeleteConfig(ConfigSelected)
                    XEN:RefreshConfigsList(ConfigsDropdown)
                end
            end
        })
        ...
        ConfigsSection:Button({
            ...
            Callback = function()
                if ConfigSelected then
                    XEN:LoadConfig(readfile(XEN.Folders.Configs .. "/" .. ConfigSelected))
                end
            end
        })
        ...
        ConfigsSection:Button({
            ...
            Callback = function()
                if ConfigSelected then
                    writefile(XEN.Folders.Configs .. "/" .. ConfigSelected, XEN:GetConfig())
                end
            end
        })
